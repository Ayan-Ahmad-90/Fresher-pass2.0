...
export default function VerifyPage() {
  ...
  const handleScan = async (result) => {
    if (!result) return;
    // @yudiel returns raw string sometimes; adapt:
  ...
  const colorFor = (s) => (s === "success" ? "#10b981" : s === "warning" ? "#f59e0b" : s === "error" ? "#ef4444" : "#fff");
  ...
  return (
    <div className="card">
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <div style={{ width: "100%", aspectRatio: "4/3", borderRadius: 8, overflow: "hidden", border: "4px solid var(--teal)" }}>
          <QrScanner
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ padding: 16, borderRadius: 8, background: colorFor(status.state), color: status.state === "idle" ? "#111" : "#fff" }}>
            <h3>{status.message}</h3>
            {status.registration && (
              <div style={{ marginTop: 12, textAlign: "left" }}>
                <p><strong>ID:</strong> {status.registration.id}</p>
                <p><strong>Name:</strong> {status.registration.name}</p>
                <p><strong>College:</strong> {status.registration.college}</p>
                <p><strong>Verified At:</strong> {status.registration.verifiedAt ?? status.registration.verified_at ?? "N/A"}</p>
              </div>
            )}
          </div>
        </div>
